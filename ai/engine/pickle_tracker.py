...
class PickleVisionTracker:
    ...
    def __init__(
        ...
        if self.use_roboflow:
            ...
            if roboflow_local:
                if get_model is None:
                    raise RuntimeError("--roboflow-local requires: pip install inference")
                ...
                if InferenceHTTPClient is None:
                    raise RuntimeError("--use-roboflow requires: pip install inference-sdk")
        ...
        self.frame_index = 0
        # Detection-health counters, read by tracker_service.status() to answer
        # "is the model seeing anything at all?" without needing console access.
        self.debug_frames = 0
        self.debug_target_class_frames = 0
        self.debug_locked_frames = 0
        self.debug_class_counts: dict[int, int] = defaultdict(int)
    ...
    def _process_frame_ultralytics(self, detect_frame):
        ...
        for cid in result.boxes.cls.int().cpu().tolist():
            self.debug_class_counts[cid] += 1
        ...
        if self.target_class_id is not None:
            ...
            if not valid_indices:
                return self._handle_missed_detection(detect_frame)
            self.debug_target_class_frames += 1
...
def parse_args():
    ...
    parser.add_argument("--roboflow-api-key", type=str, default=os.environ.get("ROBOFLOW_API_KEY"), help="Roboflow API key")
